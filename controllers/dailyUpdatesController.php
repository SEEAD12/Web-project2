...
class dailyUpdatesController{
    ...
    public function insert($img,$title,$description){
        ...
        if (isset($img) && $img['error'] === UPLOAD_ERR_OK) {
            ...
            $get = 'insert into daily_updates  set img="'.$newFileName.'" , title="'.$title.'", description="'.$description.'"';
            $query = mysqli_query($db_connection,$get);

            echo '<script>window.location="index.php?page=daily_updates "</script>';
        }
    }
